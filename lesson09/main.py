...
class Iterable:
    ...
    def __next__(self):
        if len(self.vectors) == self.current_index:
            raise StopIteration
        res = self.vectors[self.current_index]
    ...
    def generateVectors(self):
        ...
        for i in range(self.max_vectors):
            vectors.append(Vector(Point(randrange(self.max_points),
                           randrange(self.max_points))))
